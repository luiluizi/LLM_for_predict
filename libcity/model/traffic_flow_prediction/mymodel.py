...
class MyModel(AbstractTrafficStateModel):
    ...
    def forward(self, batch):
        ...
        if type(st_data_x) is list:
            ...
            region_select_out = STE_out[:, :, region_id[0]:region_id[0] + 1, :].to(torch.bfloat16)
        st_projector_out = self.st_projector(region_select_out.tranpose(1, -1))
        new_input_embeds = []
        cur_st_idx = 0
        ...
        # processed by llama
        outputs = self.llama_model.forward(input_ids=None, attention_mask=attention_mask, inputs_embeds=inputs_embeds)
        ...
        if labels is not None:
            st_pre_embs1 = hidden_states[:,
                           ...
                           :].detach().reshape(batch_size, -1, self.feature_dim, self.config.hidden_size)
            ...
            st_pre_embs2 = hidden_states[:,
                           ...
                           :].reshape(batch_size, -1, self.feature_dim, self.config.hidden_size)
            # # [4, 1, 2, 4096]-->[4, 1, 2, 128]
